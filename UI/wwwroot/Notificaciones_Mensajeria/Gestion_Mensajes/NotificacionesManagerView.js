...
import { Cat_Dependencias, Cat_Dependencias_ModelComponent } from "../../Admin/Security/OrganizationModel/Cat_Dependencias.js";
import { Tbl_Profile } from "../../Admin/Security/OrganizationModel/Tbl_Profile.js";


/**
 * @typedef {Object.<string, any>} NotificacionesManagerViewConfig
	...
	NotificationsComponent() {
		...
		if (!this.FreeNotificationComponent) {
			...
			const btnImportData = html`<input type="file"  class="btn-success" accept=".xls,.xlsx"
				onchange="${async (/** @type {{ target: { files: Blob[]; }; }} */ ev) => {
					...
					reader.onload = (e) => {
						...
						workbook.SheetNames.forEach(function (/** @type {string | number} */ sheetName) {
							// @ts-ignore
							var XL_row_object = XLSX.utils.sheet_to_row_object_array(workbook.Sheets[sheetName]);
							let fail = false
							const invitaciones = XL_row_object.filter((/** @type {any} */ i) => {
							...
							NotificationTable.Dataset = invitaciones.map((/** @type {any} */ i) => BuildDestinatario(i));

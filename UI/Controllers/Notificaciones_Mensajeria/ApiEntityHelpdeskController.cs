...
namespace API.Controllers
{
	...
	public class ApiEntityHelpdeskController : ControllerBase
	{
		...
		[HttpPost]
		[SemaphoreController]
		public List<Tbl_Comments> getTbl_CommentsWeb(Tbl_Comments Inst)
		{
			return Inst.GetComments();
		}
		[HttpPost]
		[AuthController]
		public async Task<object?> saveTbl_Comments(Tbl_Comments Inst)
		{
			...
			var response = await Inst.SaveComment(sessionKey);
			/*if (response.status == 200 )
			{
				new WSocketSignalService(_hubContext).SendCaseCommentSignal(Inst);
			}*/
			return response;
		...
		[HttpPost]
		[SemaphoreController]
		public object? saveTbl_CommentsWeb(Tbl_Comments inst)
		{
			return inst.SaveComment(HttpContext.Session.GetString("sessionKey"));
		}
		[HttpPost]
